...
function prepareAssetCaptions(fragment: HTMLElement) {
  Array.from(fragment.querySelectorAll('.screen-figure')).forEach((figure, index) => {
    const image = figure.querySelector('img');
    if (!image) return;
    image.loading = 'lazy';
    image.decoding = 'async';
    if (!image.getAttribute('alt')) {
      image.setAttribute('alt', `n8n ekran görüntüsü ${index + 1}`);
    }
  });
}
        ...
        <div className="content-column">
          ...
          <section id="iletisim" className="author-contact" aria-labelledby="iletisim-baslik">
            <div>
              ...
              <p>
                Eğitim, n8n otomasyonları ve yapay zekâ destekli iş akışlarıyla ilgili
